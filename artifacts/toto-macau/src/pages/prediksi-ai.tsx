...
import { useGetTotoMonths, useSavePrediction } from "@workspace/api-client-react";
...
function PositionCard({ pos, bbfs }: { pos: PositionAnalysis; bbfs: string[] }) {
  const [open, setOpen] = useState(false);
  const [catFilter, setCatFilter] = useState<string | null>(null);

  const ACCENT = [
    { text: "text-amber-400", bg: "bg-amber-500/15", border: "border-amber-500/30", bar: "bg-amber-500" },
    { text: "text-sky-400",   bg: "bg-sky-500/15",   border: "border-sky-500/30",   bar: "bg-sky-500" },
    { text: "text-rose-400",  bg: "bg-rose-500/15",  border: "border-rose-500/30",  bar: "bg-rose-500" },
    { text: "text-green-400", bg: "bg-green-500/15", border: "border-green-500/30", bar: "bg-green-500" },
  ][pos.position];
  ...
  return (
    <div className={cn("rounded-xl border bg-card overflow-hidden flex flex-col", ACCENT.border)}>
      {/* Header */}
      <div className={cn("px-4 py-3 flex items-center justify-between flex-wrap gap-2", ACCENT.bg)}>
        <div>
...
export default function PrediksiAIPage() {
  ...
  const result = useMemo<PredictionResult | null>(() => {
    ...
    return runPrediction(months, session);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5 px-4 pb-24 pt-6 md:pb-6">
      ...
      <div className="rounded-xl border border-border bg-card p-3">
        <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-2">PILIH SESI:</div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-border bg-card p-8 text-center text-sm text-muted-foreground">
          Belum ada data untuk sesi ini. Coba sesi lain atau refresh di halaman Home.
        </div>
      ) : (
        <>
          {/* Signal box */}
          <div className={cn(
            ...
            result.signal ? "border-green-500/40 bg-green-500/5" : "border-red-500/40 bg-red-500/5"
          )}>
            {/* Top row */}
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className={cn("text-2xl font-black font-mono tracking-tight", result.signal ? "text-green-400" : "text-red-400")}>
              ...
              <div className="flex items-center gap-5">
                <div className="text-right">
                  ...
                  <div className={cn("text-4xl font-black font-mono", confColor)}>{totalConf}%</div>
                </div>
                <div className="text-right">
                  <div className="text-[10px] text-muted-foreground">Engines</div>
                  <div className="font-mono text-xl font-black text-foreground">{result.activeEngines}</div>
            ...
            <div className="flex flex-wrap gap-6 items-start">
              <div>
                <div className="text-[10px] text-muted-foreground mb-1.5 uppercase tracking-wider">Prediksi 4D Terkuat</div>
                <div className="flex gap-1.5">
                  {["As","Kop","Kep","Ekor"].map((lbl, i) => (
                    <div key={i} className="text-center">
                      <div className="text-[8px] text-muted-foreground mb-0.5">{lbl}</div>
                      <div className={cn(
                        "flex h-12 w-10 items-center justify-center rounded-lg border font-mono font-black text-2xl",
                        result.signal
              ...
              <div>
                <div className="text-[10px] text-muted-foreground mb-1.5 uppercase tracking-wider">BBFS 4 Digit per Posisi</div>
                <div className="flex gap-3 flex-wrap">
                  {result.bbfsCandidates.map((digits, i) => (
                    <div key={i} className="text-center">
                      <div className="text-[8px] text-muted-foreground mb-0.5">{["As","Kop","Kep","Ekor"][i]}</div>
                      <div className="flex gap-0.5">
                        {digits.map((d, j) => (
                          <span key={j} className={cn(
                            "flex h-8 w-7 items-center justify-center rounded font-mono text-sm font-black border",
                            j === 0 ? "bg-primary/20 text-primary border-primary/40" :
                            j === 1 ? "bg-muted/30 text-foreground border-border" :
                            "bg-muted/15 text-muted-foreground/70 border-border/40"
                          )}>
                            {d}
                          </span>
                        ))}
