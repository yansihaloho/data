...
import {
  ...
  TrendingUp, BarChart3, Target, Award, AlertCircle,
} from "lucide-react";
...
function DigitCell({ predicted, actual, correct }: { predicted: number; actual: string | null; correct: boolean | null }) {
  if (actual === null) {
    return (
...
function StatCard({ label, value, sub, color }: { label: string; value: string | number; sub?: string; color?: string }) {
  return (
    <div className="rounded-xl border border-border bg-card p-4 text-center">
      <div className={cn("text-2xl font-black font-mono", color ?? "text-foreground")}>{value}</div>
        ...
        <>
          ...
          <div className="space-y-2 md:hidden">
            {filtered.map((p) => {
              const actualDigits = p.actual4d ? p.actual4d.split("") : null;
              return (
                <div key={p.id} className={cn("rounded-xl border bg-card p-4 space-y-3", p.digitScore === 4 ? "border-green-500/30" : "border-border")}>
